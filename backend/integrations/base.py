...
class CodeRepoAdapter(ABC):
    ...
    @abstractmethod
    async def list_commits(self, repo: str, ref: str, since: datetime, until: datetime) -> List[CodeRepoEvent]:
        """列出时间范围内的 commit"""
        raise NotImplementedError
    ...
    @abstractmethod
    async def parse_webhook(self, payload: Dict[str, Any], event_type: str) -> Optional[CodeRepoEvent]:
        """解析厂商 webhook 推送的事件"""
        raise NotImplementedError
