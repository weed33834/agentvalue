...
<p align="center">
  一个集对话、计算机操作、员工价值评估于一体的 AI 智能体平台<br/>
...
AgentValue-AI 将三类能力整合到一个平台中：

**对话** — 完整的 AI 聊天界面，支持流式输出、工具调用展示、思考过程折叠、数学公式渲染与 Markdown 导出，覆盖 ChatGPT 或 Claude.ai 上的常用交互。

**工具调用** — Agent 能执行 bash 命令、读写文件、列出目录、抓取网页、搜索代码、运行 Python 沙箱。内置工具覆盖日常操作，MCP 协议可接入 400+ 外部工具。生产环境可按需开关高危工具。

**员工价值评估** — 平台持续接收员工的多维工作数据（日报、任务进度、代码贡献、会议记录、截图、语音），经 LangGraph Agent 分析，一次推理同时产出三套视图：
...
三套视图刻意分离。同一个判断，对员工表述为"成长空间"，给主管呈现为"ROI 下滑"——措辞与立场因受众而异。所有评估必须经人工审批才能落地，这是硬约束，不是建议。
...
| `CLOUD_API_KEY` | 云端 LLM (OpenAI 兼容) | 需要真实模型时 |
| `EMBEDDING_API_KEY` | 真实 Embedding 服务 | 需要语义检索时 |
...
- v2.1 — 深度对标大厂完善管理功能矩阵：模型 Fallback/会话分析/API 健康/数据集/LLM 评测/RAG 评测/人工标注/SSO/模板市场/NL2SQL/文档解析 + 19 项安全加固
...
如有想推进的方向，欢迎到 [GitHub Issues](https://github.com/weed33834/agentvalue/issues) 提议。
